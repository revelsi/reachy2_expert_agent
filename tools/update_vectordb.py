...
from src.utils.db_utils import VectorStore
from src.utils.doc_utils import load_documents_from_json
from src.utils.embedding_utils import (EmbeddingFunction, EmbeddingGenerator,
                                   prepare_documents_for_db)


def process_json_file(
    filepath: str, db: VectorStore, embedding_generator: EmbeddingGenerator
) -> bool:
    ...
    try:
        ...
        try:
            ...
            if count != len(texts):
                print(
                    f"Warning: Expected {len(texts)} documents but found {count} in collection"
                )
            else:
                print(
                    f"Successfully added {count} documents to collection '{collection_name}'"
                )
            return True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Update vector database with document embeddings"
    )
    parser.add_argument(
        "--test", action="store_true", help="Use lighter model for testing"
    )
    ...
    try:
        ...
        db = VectorStore(persist_directory="data/vectorstore")
