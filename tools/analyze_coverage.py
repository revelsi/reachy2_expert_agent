...
def analyze_api_coverage():
    ...
    processed_modules = load_json_file(
        os.path.join(OUTPUT_DIR, "api_docs_modules.json")
    )
    processed_classes = load_json_file(
        os.path.join(OUTPUT_DIR, "api_docs_classes.json")
    )
    processed_functions = load_json_file(
        os.path.join(OUTPUT_DIR, "api_docs_functions.json")
    )
    ...
    for item_type in raw_items.keys():
        ...
        print(
            f"Coverage: {len(processed_set)}/{len(raw_set)} ({(len(processed_set)/len(raw_set)*100 if raw_set else 0):.1f}%)"
        )
...
def analyze_example_coverage():
    ...
    if raw_sdk:
        raw_examples = {
            ex.get("metadata", {}).get("source", "unknown") for ex in raw_sdk
        }
        processed_examples = {
            chunk.get("metadata", {}).get("source", "unknown")
            for chunk in processed_sdk
        }
    ...
    processed_tutorials = load_json_file(
        os.path.join(OUTPUT_DIR, "reachy2_tutorials.json")
    )
    ...
    if raw_tutorials:
        raw_tut = {
            tut.get("metadata", {}).get("source", "unknown") for tut in raw_tutorials
        }
        processed_tut = {
            chunk.get("metadata", {}).get("source", "unknown")
            for chunk in processed_tutorials
        }
