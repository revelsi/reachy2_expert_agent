...
CACHE_DIR = os.path.join("data/cache", "raw_docs")
...
def clone_or_update_repo():
    ...
    try:
        ...
        if not os.path.exists(REPO_DIR):
            ...
            # Add remote
            subprocess.run(
                ["git", "remote", "add", "origin", GIT_URL], cwd=REPO_DIR, check=True
            )
            ...
            subprocess.run(
                ["git", "reset", "--hard", "origin/main"], cwd=REPO_DIR, check=True
            )
            print("Repository updated successfully")

        # Update cache
        save_cache({"last_update": datetime.now().isoformat(), "repo_path": REPO_DIR})
