...
EXTERNAL_DOCS_DIR = "external_docs/documents"
...
EXAMPLES_SOURCE_DIR = os.path.join(REPO_DIR, "src", "examples")  # Examples are in src/examples
TUTORIALS_SOURCE_DIR = os.path.join(RAW_DOCS_DIR, "reachy2_tutorials")  # Tutorials from separate repo
...
def extract_sdk_documentation() -> List[Dict]:
    ...
    def process_function_def(node: ast.FunctionDef, module_name: str, parent_class: str = None) -> Dict:
        ...
        if (not node.name.startswith('_') or 
            ...
            return {
                ...
                'decorators': [d.id if isinstance(d, ast.Name) else d.func.id 
                             for d in node.decorator_list 
    ...
    def process_class_def(node: ast.ClassDef, module_name: str) -> Dict:
        ...
        class_doc = {
            ...
            'bases': [base.id if isinstance(base, ast.Name) else base.value.id 
    ...
    for root, _, files in os.walk(SDK_SOURCE_DIR):
        for file in files:
            ...
            file_path = os.path.join(root, file)
            module_name = os.path.relpath(file_path, SDK_SOURCE_DIR).replace('/', '.').replace('.py', '')
